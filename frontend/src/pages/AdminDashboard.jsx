...
import LayoutUploader from '../components/admin/LayoutUploader' // NEW IMPORT
import ContactManager from '../components/admin/ContactManager'
import { unitAPI, contactAPI } from '../services/api'
import GalleryManager from '../components/admin/GalleryManager'
import AdminApprovalPage from '../components/admin/AdminApprovalPage' // NEW IMPORT
...
const AdminDashboard = () => {
  ...
  const [showLayoutUploader, setShowLayoutUploader] = useState(false) // NEW STATE
  ...
  useEffect(() => {
    ...
    // Note: User management data is fetched by AdminApprovalPage component itself
  }, [activeTab])
  ...
  // NEW: Handle layout management
  const handleManageLayout = (unit) => {
    console.log('Managing layout for unit:', unit.unitNumber)
    setSelectedUnit(unit)
    setShowLayoutUploader(true)
  }

  const handleCloseLayoutUploader = () => {
    setShowLayoutUploader(false)
    setSelectedUnit(null)
  }
  ...
  // NEW: Handle layout updated
  const handleLayoutUpdated = async () => {
    // Refresh the specific unit data after layout is updated
    if (selectedUnit) {
      try {
        const response = await unitAPI.getById(selectedUnit._id)
        const updatedUnit = response.data
        ...
        // Update the selected unit for the layout uploader
        setSelectedUnit(updatedUnit)
        
        console.log('Unit layout refreshed successfully')
      } catch (error) {
        console.error('Error refreshing unit data:', error)
      }
    }
  ...
  // Show loading state (only for units and contacts tabs)
  if (loading && (activeTab === 'units' || activeTab === 'contacts')) {
    return (
  ...
  // Show error state (only for units and contacts tabs)
  if (error && (activeTab === 'units' || activeTab === 'contacts')) {
    return (
      <div style={{ padding: '2rem', textAlign: 'center' }}>
      ...
      <div style={{ 
        ...
        <div>
          ...
          <p style={{ color: '#7f8c8d', margin: 0 }}>
            Connected to backend • Manage units, users, and contact inquiries
          </p>
        </div>
        ...
        <div style={{ display: 'flex', gap: '0' }}>
          ...
          <button
            ...
            style={{
              ...
              fontSize: '16px',
              fontWeight: '500',
              borderBottom: activeTab === 'users' ? '3px solid #000' : '3px solid transparent',
              transition: 'all 0.3s ease'
            }}
          >
            User Management
          ...
          <div style={{ 
            ...
            <div style={{ 
              ...
              <h3 style={{ color: '#f57c00', margin: 0 }}>
                {units.filter(u => u.layoutImage).length}
              </h3>
              <p style={{ margin: 0, fontSize: '0.9rem' }}>With Layouts</p>
            </div>
          </div>
                    ...
                    <tr key={unit._id} style={{ borderBottom: '1px solid #dee2e6' }}>
                      ...
                      <td style={{ padding: '1rem' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                          <span style={{ 
                            fontSize: '0.9rem',
                            color: unit.layoutImage ? '#28a745' : '#6c757d'
                          }}>
                            {unit.layoutImage ? (
                              <span title={unit.layoutImage.includes('cloudinary.com') ? 'Cloudinary' : 'Local'}>
                                {unit.layoutImage.includes('cloudinary.com') ? '☁️ Yes' : '💾 Yes'}
                              </span>
                            ) : (
                              'No'
                            )}
                          ...
                          <button
                            onClick={() => handleManageLayout(unit)}
                            style={{
        ...
        <div style={{
          ...
          <div style={{
            ...
            <div style={{
              ...
              <h2 style={{ margin: 0 }}>
                Manage Layout - Unit {selectedUnit?.unitNumber}
              </h2>
              <button
                onClick={handleCloseLayoutUploader}
                style={{
                  backgroundColor: '#6c757d',
                  color: 'white',
                  border: 'none',
